...
def get_avro_steps(dataset_list):
    return [
        step_bash_cmd("a. Download Verbatim AVRO", f" /tmp/download-datasets-avro.sh {ala_config.S3_BUCKET_AVRO} {dataset_list}")
    ]
...
def get_pre_image_steps(dataset_list, override_uuid_percentage=False):
    ...
    return [
        ...
        step_bash_cmd("e.2 Access Controls", f" la-pipelines access-controls {dataset_list}"),
    ]


def get_load_image_steps(dataset_list):
    return [
        step_bash_cmd("f. Image loading", f" la-pipelines image-load {dataset_list}"),
    ]
...
with DAG(
    ...
    cluster_creator = EmrCreateJobFlowOperator(
        ...
        job_flow_overrides=cluster_setup.get_small_cluster(DAG_ID, "bootstrap-ingest-small-actions.sh"),
